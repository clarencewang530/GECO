...
python3 train.py --outdir=./training-runs/zero123plus-dtype --data='zero123plus:80:1' --cond=0 --gen=same --arch=unet_zero123plus \
 --glr=1e-5 --loss_type=vsd --cfg_tchr=4.0 --cfg_stu=1.0 \
  --max_iters=20000 --suffix 'l2' --batch=2 --gpus=2 \
  --lr=1e-5 --snap=200 --img-snap=50 --timestamp=True --ft_all=True --model_path='sudo-ai/zero123plus-v1.1' --custom_pipeline='../pipelines/zero123plus.py' --init_t=950 --dtype=fp32 --root_dir=/scratch/inf0/user/lliu/lvis_six_random --use_lgm=True --use_vsd=False \
