...
class Zero123Config():
    ...
    cond_image_path: str = "./data/lysol.png"
    cond_elevation_deg: float = 0.0
    cond_azimuth_deg: float = -90.0
...
class Zero123Pipeline(DiffusionPipeline):
    ...
    def decode_latents(self, latents):
        ...
        image = torch.nn.functional.interpolate(image, (256, 256))
    ...
    def prepare_image_embeddings(self) -> None:
        ...
        with torch.no_grad():
            ...
            self.image_latents: Float[Tensor, "1 4 Hl Wl"] = (
                self.vae_encode(self.vae, image * 2.0 - 1.0, mode=True)
                / self.vae.config.scaling_factor
            )
        return self.image_latents, self.clip_image_embeddings
    ...
    def extract_clip_image_embeddings(
            ...
            images_processed = self.feature_extractor(
                images=images_pil, return_tensors="pt"
            ).pixel_values.to(device=self.device)
            clip_image_embeddings = self.image_encoder(images_processed).image_embeds
            return clip_image_embeddings.to(images.dtype)
        
    def get_image_camera_embeddings_batch(self, image, cond_elevation_deg, cond_azimuth_deg, cond_camera_distance, elevation_deg, azimuth_deg, camera_distances):
        batch_size = elevation_deg.shape[0] // image.shape[0]
        with torch.no_grad():
            clip_image_embeddings = self.extract_clip_image_embeddings(image)
            image_latents = self.vae_encode(self.vae, image * 2.0 - 1.0, mode=True) / self.vae.config.scaling_factor # (B, 4, 32, 32)
        camera_embeddings = torch.stack([
            torch.deg2rad(cond_elevation_deg - elevation_deg),
            torch.sin(torch.deg2rad(azimuth_deg - cond_azimuth_deg)),
            torch.cos(torch.deg2rad(azimuth_deg - cond_azimuth_deg)),
            camera_distances - cond_camera_distance,
        ], dim=-1)[:, None, :] # (B, 4) -> (B, 1, 4)
        image_camera_embeddings = self.clip_camera_projection(
            torch.cat(
                [
                    clip_image_embeddings.unsqueeze(1), # (B, D) -> (B, 1, D)
                    camera_embeddings,
                ],
                dim=-1
            )
        )
        return image_camera_embeddings, image_latents
    ...
    def forward_unet(
        ...
        t: Int[Tensor, "..."],
        encoder_hidden_states: Float[Tensor, "..."] = None,
        ...
        latent_model_input = torch.cat(
            [
                torch.cat([latents] * 2, dim=0),
                torch.cat(
                    [
                        image_latents.repeat(batch_size, 1, 1, 1),
                        torch.zeros_like(image_latents).repeat(batch_size, 1, 1, 1),
                    ],
                    dim=0,
                ),
            ],
    ...
    def gen_from_cond(
        ...
        unet = self.unet if unet == None else unet
        B = image_camera_embeddings.shape[0]
        latents = torch.randn((B, 4, 32, 32), device=image_latents.device)
        encoder_hidden_states = torch.cat(
            [
                image_camera_embeddings,
                torch.zeros_like(image_camera_embeddings),
        ...
        for t in self.scheduler.timesteps:
            latent_model_input = torch.cat(
                [
                    ...
                    torch.cat(
                        [
                            image_latents.repeat(B, 1, 1, 1),
                            torch.zeros_like(image_latents).repeat(B, 1, 1, 1),
                        ],
                        dim=0,
                    ),
                ],
            ...
            noise_pred = unet(
                latent_model_input.to(image_latents.dtype),
                torch.cat([t.reshape(1).repeat(B).to(image_latents.device)] * 2, dim=0),
                encoder_hidden_states=encoder_hidden_states.to(image_latents.dtype)
            ).sample
