...
class SingleSplatterImage(nn.Module):
    def __init__(
        self,
        opt: Options,
    ):
        ...
        self.splatter_out = nn.Parameter(torch.randn((1, self.opt.num_input_views, 14, self.opt.splat_size, self.opt.splat_size), requires_grad=True))
        self.is_init = True
    ...
    def prepare_default_rays(self, device, elevation, azimuth):
        ...
        # ], axis=0) # [4, 4, 4]
        cams = [orbit_camera(ele, azi, radius=self.opt.cam_radius) for (ele, azi) in zip(elevation, azimuth)]
    ...
    def forward_gaussians(self, images):
        ...
        if self.is_init:
            ...
            self.splatter_out = nn.Parameter(x.clone().detach(), requires_grad=True)
            self.is_init = False
        x = self.splatter_out
